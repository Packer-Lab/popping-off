    ...
    def significance_analysis(self,X,trialX,n_shuffles=100,n_splits=100,n_consecutive=1,axis=None,full=False):
        ...
        def compute_mean_score(X,trialX,n_splits):
            ...
            for shuffle in range(n_splits):
                ...
                trainX, validX = self.train_test_split(X,trialX)  # both shape of X
                ...
                for key in keys:
                    ...
                    # for ax in axes:  # the issue in this loop is that the loop over axes is not consistent when overwriting trainZ[key]/validZ[key]
                    ...
                    # reshape
                    # if len(X.shape)-2 in axset and axis is not None:  # this should be time axis for the case where X.shape = (N, S, T)?
                    axis_time = list(self.marginalizations['t'])[0]  # find time axis directly 
